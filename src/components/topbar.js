  ...
  return `<header class="fixed top-0 right-0 left-64 h-16 bg-surface/80 backdrop-blur-xl z-40 flex items-center justify-between px-8 border-b border-outline-variant/20">
    ...
    <div class="flex items-center gap-3">
      ...
      <button id="settings-btn" title="Location & Language Settings"
        class="flex items-center gap-2 px-3 py-1.5 bg-surface-container-low rounded-full hover:bg-surface-container transition-colors border border-outline-variant/20 text-xs font-bold text-on-surface-variant">
        <span class="material-symbols-outlined text-primary text-sm">location_on</span>
        <span id="topbar-location" class="max-w-[140px] truncate">${location}</span>
        <span class="material-symbols-outlined text-outline text-xs">edit</span>
      </button>

      <!-- Language badge -->
      <span class="w-8 h-8 flex items-center justify-center text-outline rounded-full bg-surface-container-low border border-outline-variant/30 text-xs font-bold uppercase" title="Current language">${userPrefs.language}</span>
