...
// ─── NPK bar helper — must be defined before renderRecommendations uses it ───
  ...
  return `<main class="ml-64 pt-16 min-h-screen page-enter">
    <div class="p-8 max-w-7xl mx-auto">
        ...
        <div class="bg-surface-container-lowest p-6 rounded-2xl shadow-sm border border-outline-variant/10">
          ...
          <div class="space-y-4">
            ...
            <div>
              ...
              <input id="rec-district" type="text" value="${userPrefs.district || userPrefs.city || ''}" placeholder="e.g. Nanded, Pune" class="w-full bg-surface-container-low border-none rounded-xl text-sm py-2.5 px-3 focus:ring-2 focus:ring-primary/20"/>
            </div>
            <div>
      ...
      <div id="rec-loading" class="hidden py-16 text-center">
        ...
        <p class="text-sm text-outline mt-1">This may take 10–20 seconds</p>
      </div>
...
export function initRecommendations() {
  ...
  async function loadNews() {
    ...
    try {
      ...
      if (d.news?.length) {
        ...
        el.innerHTML = '<p class="text-sm text-outline col-span-4 p-4">No news available.</p>';
      }
    } catch {
  ...
  async function fetchRecommendations() {
    ...
    try {
      const data = await getRecommendations({
        ...
        soilType,
        farmSize,
        // Send soilData as nested object (worker reads from here)
      ...
      if (data.error && !data.recommendedCrops?.length) {
        resultsEl.innerHTML = `
          <div class="py-8 px-6 bg-error/10 border border-error/20 rounded-2xl text-center">
            ...
            <p class="text-sm text-on-surface-variant">${data.error}</p>
            <button onclick="document.getElementById('get-recommendations').click()" class="mt-4 px-6 py-2 bg-primary text-white rounded-xl text-sm font-bold hover:opacity-90">Retry</button>
          </div>`;
        return;
      }
            ...
            ${crops.map((c, i) => `
              <div class="bg-surface-container-lowest p-5 rounded-2xl shadow-sm border border-outline-variant/10 hover:shadow-md transition-shadow">
                <div class="flex items-start gap-3 mb-3">
                  <div class="w-10 h-10 rounded-xl flex items-center justify-center text-white font-bold text-sm flex-shrink-0" style="background:hsl(${120 + i * 30}, 40%, 35%)">
                    ${i + 1}
                  </div>
                  <div class="min-w-0">
                    <h4 class="font-bold text-primary text-base truncate">${c.name || c.crop || 'Crop'}</h4>
                    <span class="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full ${suitabilityColor[c.suitability] || 'text-outline bg-surface-container'}">
                      ${(c.suitability || 'suitable').replace(/-/g, ' ')}
                    </span>
                  </div>
                </div>
                <p class="text-xs text-on-surface-variant leading-relaxed mb-3 line-clamp-3">${c.reason || c.description || ''}</p>
                <div class="grid grid-cols-2 gap-2">
                  ${c.expectedYield && c.expectedYield !== '—' ? `<div class="p-2 bg-primary-fixed/20 rounded-lg text-center">
                    <p class="text-[10px] font-bold text-outline uppercase">Yield</p>
                    <p class="text-xs font-bold text-primary">${c.expectedYield}</p>
                  </div>` : ''}
                  ${c.marketPrice && c.marketPrice !== '—' ? `<div class="p-2 bg-secondary-fixed/20 rounded-lg text-center">
                    <p class="text-[10px] font-bold text-outline uppercase">Price</p>
                    <p class="text-xs font-bold text-secondary">${c.marketPrice}</p>
                  </div>` : ''}
                  ${c.waterNeeds && c.waterNeeds !== '—' ? `<div class="p-2 bg-tertiary-fixed/20 rounded-lg text-center">
                    <p class="text-[10px] font-bold text-outline uppercase">Water</p>
                    <p class="text-xs font-bold text-tertiary">${c.waterNeeds}</p>
                  </div>` : ''}
                  ${c.duration && c.duration !== '—' ? `<div class="p-2 bg-surface-container-low rounded-lg text-center">
            ...
            <div class="space-y-2">
              ${fert.basalDose.map(f => `
                <div class="p-3 bg-primary-fixed/10 rounded-xl">
                  ...
                  <p class="text-xs text-on-surface-variant">${f.quantity}${f.timing ? ` • ${f.timing}` : ''}</p>
                </div>`).join('')}
            </div>
          </div>`;
        }
            ...
            <div class="space-y-2">
              ${fert.micronutrients.map(m => `
                <div class="p-3 bg-tertiary-fixed/10 rounded-xl">
                  ...
                  <p class="text-xs text-on-surface-variant">${m.application ? `${m.application} • ` : ''}${m.quantity}</p>
                </div>`).join('')}
            </div>
          </div>`;
        }
      ...
      if (!html) {
        html = '<p class="text-sm text-outline text-center py-8">No recommendations generated. Try adjusting your inputs and retry.</p>';
      }
      ...
      resultsEl.innerHTML = `
        <div class="py-8 px-6 bg-error/10 border border-error/20 rounded-2xl text-center">
          <span class="material-symbols-outlined text-error text-4xl mb-3 block">error_outline</span>
          <p class="font-bold text-error mb-2">Request failed</p>
  ...
  // Refresh button reloads news and clears old results
  document.getElementById('rec-refresh')?.addEventListener('click', () => {
    document.getElementById('rec-results').innerHTML = '';
    loadNews();
  });

  // Listen for prefs changes — update state/district selectors
