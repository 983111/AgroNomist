...
function nutrientBar(label, value, low, high, color = '#123b2a') {
  ...
  return `<div>
    <div class="flex justify-between items-baseline mb-1.5">
      <p class="text-[42px] font-extrabold leading-none ${status === 'LOW' ? 'text-secondary' : 'text-primary'}">${label}</p>
...
function recommendationItem(crop, idx) {
  ...
  return `<div class="flex items-center gap-3 bg-white/10 rounded-2xl p-4">
    <div class="w-12 h-12 rounded-xl bg-white/10 flex items-center justify-center"><span class="material-symbols-outlined text-primary-fixed">${icon}</span></div>
    <div class="flex-1 min-w-0">
      <p class="font-bold text-2xl text-white leading-tight">${crop?.name || `Crop ${idx + 1}`}</p>
      <p class="text-sm text-primary-fixed-dim">Match Probability: ${prob}%</p>
    </div>
  </div>`;
}

function statTile(icon, label, value) {
  return `<div class="bg-surface-container-lowest rounded-2xl px-6 py-5 border border-outline-variant/20 shadow-sm">
    <div class="flex items-center gap-3">
      ...
      <div>
        ...
        <p class="text-3xl font-extrabold text-primary mt-1">${value}</p>
      </div>
    </div>
  </div>`;
}
...
export function renderRecommendations() {
  return `<main class="ml-64 pt-16 min-h-screen page-enter bg-background">
    <div class="px-8 py-8 max-w-[1400px] mx-auto space-y-8">
      <div class="flex flex-wrap justify-between items-start gap-4">
        <div>
          <h2 class="font-headline text-6xl leading-tight font-extrabold text-primary">Soil Intelligence <span class="text-secondary">&amp; Analytics</span></h2>
          <p class="text-3xl mt-3 text-on-surface-variant">Integrating hyperspectral soil analysis with predictive yield modeling.</p>
        </div>
        <div class="text-right">
          <p class="text-xs tracking-[0.25em] font-bold text-outline uppercase">Last Sensor Sync</p>
          <p class="text-4xl font-extrabold text-primary" id="soil-last-sync">--:--</p>
        </div>
      </div>

      <section class="bg-surface-container-lowest rounded-3xl border border-outline-variant/20 p-6 shadow-sm">
      ...
      <div id="soil-loading" class="hidden bg-primary/5 rounded-2xl p-5 text-primary font-bold">Analyzing live soil data…</div>
      <div id="soil-error"></div>

      <section id="soil-results" class="grid grid-cols-12 gap-6">
        <div class="col-span-8 bg-surface-container-lowest rounded-3xl p-8 border border-outline-variant/20 shadow-sm">
          <div class="flex items-center justify-between mb-5"><h3 class="font-headline text-5xl font-extrabold text-primary">Macronutrient Profile</h3><p class="px-4 py-2 rounded-xl text-xl font-bold bg-primary-fixed text-primary" id="soil-optimal-ph">Optimal pH: 6.8</p></div>
          <p class="text-sm font-bold tracking-[0.2em] text-outline uppercase mb-6">Chemical Composition Analysis</p>
          <div class="grid grid-cols-3 gap-6" id="soil-nutrients"></div>
          <div class="mt-8 bg-surface-container-low rounded-2xl p-5 border-l-4 border-secondary">
            <p class="font-bold text-2xl text-primary" id="soil-insight-title">AI Insight</p>
            <p class="text-on-surface-variant mt-2 text-lg" id="soil-insight-text">Generate report to see optimized nutrient and yield guidance.</p>
          </div>
        </div>

        <div class="col-span-4 rounded-3xl p-8 bg-gradient-to-b from-primary to-[#0c5a3d] text-white shadow-xl">
          <p class="inline-block px-3 py-1 rounded-full bg-white/10 text-[11px] font-bold tracking-[0.14em]">India Context</p>
          <h3 class="font-headline text-5xl font-extrabold mt-4">Crop Recommendation</h3>
          <div class="mt-5 space-y-4" id="soil-recommendations"></div>
          <button id="soil-report-btn" class="w-full mt-6 py-3 rounded-xl bg-white text-primary font-extrabold tracking-[0.12em] uppercase">Generate Full Report</button>
        </div>

        <div class="col-span-8 bg-surface-container-lowest rounded-3xl p-8 border border-outline-variant/20 shadow-sm">
          <h3 class="font-headline text-5xl font-extrabold text-primary">Fertilizer Planner</h3>
          <p class="text-sm font-bold tracking-[0.2em] text-outline uppercase">Interactive Cost vs Yield Optimizer</p>
          <div class="mt-7">
            <label class="block text-sm font-bold mb-2">Fertilizer Intensity (<span id="soil-intensity-text">85</span>%)</label>
            <input id="soil-intensity" type="range" min="40" max="120" value="85" class="w-full"/>
          </div>
          <div class="grid grid-cols-2 gap-4 mt-6">
            <div class="bg-surface-container rounded-2xl p-5"><p class="text-xs uppercase tracking-[0.12em] font-bold text-outline">Projected Cost</p><p class="text-5xl mt-3 font-extrabold text-primary" id="soil-projected-cost">₹24,500/acre</p></div>
            <div class="bg-secondary-fixed/40 rounded-2xl p-5"><p class="text-xs uppercase tracking-[0.12em] font-bold text-outline">Estimated Yield Gain</p><p class="text-5xl mt-3 font-extrabold text-secondary" id="soil-yield-gain">+18.4%</p></div>
          </div>
        </div>
